...
def create_gui():
    ...
    tk.Label(root, text="Open Port Finder", font=("Helvetica", 16, "bold"), bg='#f0f0f0').pack(pady=10)

    tk.Label(root, text="Enter the IP address, hostname, or full URL:", font=("Helvetica", 12), bg='#f0f0f0').pack(pady=5)
    entry_target = tk.Entry(root, width=60, font=("Helvetica", 12))
    entry_target.pack(pady=5)

    tk.Label(root, text="Enter the range of ports to scan:", font=("Helvetica", 12), bg='#f0f0f0').pack(pady=5)

    frame_ports = tk.Frame(root, bg='#f0f0f0')
    frame_ports.pack(pady=5)

    tk.Label(frame_ports, text="Start Port:", font=("Helvetica", 12), bg='#f0f0f0').pack(side=tk.LEFT, padx=5)
    entry_start_port = tk.Entry(frame_ports, width=10, font=("Helvetica", 12))
    entry_start_port.pack(side=tk.LEFT, padx=5)

    tk.Label(frame_ports, text="End Port:", font=("Helvetica", 12), bg='#f0f0f0').pack(side=tk.LEFT, padx=5)
    entry_end_port = tk.Entry(frame_ports, width=10, font=("Helvetica", 12))
    entry_end_port.pack(side=tk.LEFT, padx=5)

    tk.Button(root, text="Start Scan", font=("Helvetica", 12, "bold"), bg="#007bff", fg="#ffffff", command=start_scan).pack(pady=10)

    text_results = scrolledtext.ScrolledText(root, wrap=tk.WORD, width=90, height=15, font=("Helvetica", 12))
